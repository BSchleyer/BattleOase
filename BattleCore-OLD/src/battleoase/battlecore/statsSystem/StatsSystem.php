...
class StatsSystem
{
    ...
    {
        BattleCore::getInstance()->getMysqlConnection()->query("CREATE TABLE Stats.{$game} ( `id` INT NOT NULL AUTO_INCREMENT , `player_name` VARCHAR(16) NOT NULL , `kills` INT(255) NOT NULL , `deaths` INT(255) NOT NULL , `elo` INT(255) NOT NULL , `wins` INT(255) NOT NULL , `loses` INT(255) NOT NULL, `date` DATE NOT NULL DEFAULT CURRENT_TIMESTAMP  , PRIMARY KEY (`id`, `player_name`)) ENGINE = InnoDB;");
    }

    private function editStatsProfile($name, string $game, int $kills = 0, int $deaths = 0, int $elo = 0, int $wins = 0, int $loses = 0)
    {
        BattleCore::getInstance()->getMysqlConnection()->query("INSERT INTO Stats.{$game} (player_name, kills, deaths, elo, wins, loses) VALUES ('$name', '$kills', '$deaths', '$elo', '$wins', '$loses');");
    }
    ...
    public function saveSkin($player, Skin $skin)
    {
        ...
        BattleCore::getInstance()->getMysqlConnection()->query("INSERT INTO Stats.Skins(`player_name`, `skin_data`, `cape_data`,`geometry_name`, `geometry_data`) VALUES ('$name', '$skinData', '$capedata', '$geometryName', '$geometryData')");
        BattleCore::getInstance()->getMysqlConnection()->query("UPDATE Stats.Skins SET `skin_data` = '$skinData' ,`cape_data` = '$capedata' , `geometry_name` = '$geometryName' ,`geometry_data` = '$geometryData' WHERE `player_name` = '$name'");
    }
	...
	{
		$result = BattleCore::getInstance()->getMysqlConnection()->query("SELECT * FROM Stats.Skins WHERE player_name='$name'");
    ...
    {
        $return = [];
        $query = BattleCore::getInstance()->getMysqlConnection()->query("SHOW TABLES IN Stats");
        if($query->num_rows > 0) {
    ...
    {
        $return = [];
        $result = BattleCore::getInstance()->getMysqlConnection()->query("SELECT player_name, SUM(wins) AS wins FROM Stats.{$game} WHERE date BETWEEN (CURDATE() - INTERVAL 30 DAY) AND CURDATE() GROUP BY player_name ORDER BY SUM(elo) DESC LIMIT {$limit}");
    ...
    {
        $return = [];
        $result = BattleCore::getInstance()->getMysqlConnection()->query("SELECT player_name, SUM(kills) AS kills FROM Stats.{$game} WHERE date BETWEEN (CURDATE() - INTERVAL 30 DAY) AND CURDATE() GROUP BY player_name ORDER BY SUM(elo) DESC LIMIT {$limit}");
        if($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
    ...
    public function getTopElo(string $game, int $limit)
    {
        ...
        $result = BattleCore::getInstance()->getMysqlConnection()->query("SELECT player_name, SUM(elo) AS elo FROM Stats.{$game} WHERE date BETWEEN (CURDATE() - INTERVAL 30 DAY) AND CURDATE() GROUP BY player_name ORDER BY SUM(elo) DESC LIMIT {$limit}");
        if($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
    ...
    public function getElo($player, string $game, int $days = 30000)
    {
        ...
        $result = BattleCore::getInstance()->getMysqlConnection()->query("SELECT SUM(elo) AS elo FROM Stats.{$game} WHERE player_name='$player' AND date BETWEEN (CURDATE() - INTERVAL $days DAY) AND CURDATE()");
        if($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                return $row["elo"];
            }
    ...
    public function getKill($player, string $game, int $days = 30000)
    {
        ...
        $result = BattleCore::getInstance()->getMysqlConnection()->query("SELECT SUM(kills) AS kills FROM Stats.{$game} WHERE player_name='$player' AND date BETWEEN (CURDATE() - INTERVAL $days DAY) AND CURDATE()");
        if($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                return $row["kills"];
            }
    ...
    public function getDeath($player, string $game, int $days = 30000)
    {
        ...
        $result = BattleCore::getInstance()->getMysqlConnection()->query("SELECT SUM(deaths) AS deaths FROM Stats.{$game} WHERE player_name='$player' AND date BETWEEN (CURDATE() - INTERVAL $days DAY) AND CURDATE()");
        if($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                return $row["deaths"];
            }
    ...
    public function getLoses($player, string $game, int $days = 30000)
    {
        ...
        $result = BattleCore::getInstance()->getMysqlConnection()->query("SELECT SUM(loses) AS loses FROM Stats.{$game} WHERE player_name='$player' AND date BETWEEN (CURDATE() - INTERVAL $days DAY) AND CURDATE()");
        if($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                return $row["loses"];
            }
    ...
    public function getWins($player, string $game, int $days = 30000)
    {
        ...
        $result = BattleCore::getInstance()->getMysqlConnection()->query("SELECT SUM(wins) AS wins FROM Stats.{$game} WHERE player_name='$player' AND date BETWEEN (CURDATE() - INTERVAL $days DAY) AND CURDATE()");
        if($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                return $row["wins"];
            }
