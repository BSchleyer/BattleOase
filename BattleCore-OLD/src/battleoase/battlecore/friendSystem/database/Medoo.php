...
class Medoo{
	...
	public function __construct(array $options){
		...
		if (isset($options['pdo'])) {
			if (!$options['pdo'] instanceof PDO) {
				throw new InvalidArgumentException('Invalid PDO objects supplied');
			}
		...
		if (isset($options['dsn'])) {
			...
			switch ($this->type) {
				...
				case 'oracle':
					$attr = [
						'driver' => 'oci',
						'dbname' => $options['server'] ? '//' . $options['server'] . ($is_port ? ':' . $port
								: ':1521') . '/' . $options['database_name'] : $options['database_name'],
					];
		...
		$this->dsn = $dsn;
		try {
			$this->pdo = new PDO($dsn, $options['username'] ?? null, $options['password'] ?? null, $option);
			foreach ($commands as $value) {
				$this->pdo->exec($value);
			}
	...
	protected function generate($query, $map){
		...
		$query = preg_replace('/"([a-zA-Z0-9_]+)"/i', $identifier[$this->type] ?? '"$1"', $query);
		foreach ($map as $key => $value) {
			if ($value[1] === PDO::PARAM_STR) {
				$replace = $this->quote($value[0]);
			} else if ($value[1] === PDO::PARAM_NULL) {
				$replace = 'NULL';
			} else if ($value[1] === PDO::PARAM_LOB) {
				$replace = '{LOB_DATA}';
			} else {
				$replace = $value[0];
			}
	...
	protected function buildRaw($raw, &$map): array|bool|string|null
    {
		if (!$this->isRaw($raw)) {
			return false;
		}
		$query = preg_replace_callback('/((FROM|TABLE|INTO|UPDATE)\s*)?\<([a-zA-Z0-9_\.]+)\>/i', function ($matches){
			if (!empty($matches[2])) {
	...
	public function quote($string): bool|string
    {
	...
	protected function typeMap($value, $type): array
    {
		$map = [
			'NULL'     => PDO::PARAM_NULL,
	...
	protected function columnPush(&$columns, &$map): string
    {
		if ($columns === '*') {
			return $columns;
		}
		...
		foreach ($columns as $key => $value) {
			...
			} else if (!is_int($key) && $raw = $this->buildRaw($value, $map)) {
	...
	protected function arrayQuote($array): string
    {
		$stack = [];
	...
	protected function innerConjunct($data, $map, $conjunctor, $outer_conjunctor): string
    {
		$stack = [];
			...
			if (is_int($key) && preg_match('/([a-zA-Z0-9_\.]+)\[(?<operator>\>\=?|\<\=?|\!?\=)\]([a-zA-Z0-9_\.]+)/i', $value, $match)) {
				...
				if (isset($match['operator'])) {
					...
					if (in_array($operator, ['>', '>=', '<', '<='])) {
						...
						} else if ($raw = $this->buildRaw($value, $map)) {
							$condition .= $raw;
						} else {
							$condition .= $map_key;
						...
						foreach ($value as $index => $item) {
							...
							$like_clauses[] = $column . ($operator === '!~' ? ' NOT'
									: '') . ' LIKE ' . $map_key . 'L' . $index;
						...
						if ($type === 'array') {
							...
							$data_type = (is_numeric($value[0]) && is_numeric($value[1])) ? PDO::PARAM_INT
								: PDO::PARAM_STR;
	...
	protected function whereClause($where, &$map): string
    {
		...
		if (is_array($where)) {
			...
			if (isset($where['MATCH']) && $this->type === 'mysql') {
				...
				if (is_array($MATCH) && isset($MATCH['columns'], $MATCH['keyword'])) {
					...
					$where_clause .= ($where_clause !== '' ? ' AND '
							: ' WHERE') . ' MATCH (' . $columns . ') AGAINST (' . $map_key . $mode . ')';
				}
			}
			if (isset($where['GROUP'])) {
				...
				} else if ($raw = $this->buildRaw($GROUP, $map)) {
			...
			if (isset($where['ORDER'])) {
				...
				} else if ($raw = $this->buildRaw($ORDER, $map)) {
		...
		} else if ($raw = $this->buildRaw($where, $map)) {
			$where_clause .= ' ' . $raw;
		}
		return $where_clause;
	}
						...
						if (isset($relation[0])) {
							...
							foreach ($relation as $key => $value) {
								$joins[] = (strpos($key, '.') > 0
										...
										: // For ['column1' => 'column2']
										$table . '."' . $key . '"') . ' = ' . $this->tableQuote($match['alias'] ?? $match['table']) . '."' . $value . '"';
							}
	...
	protected function columnMap($columns, &$stack){
		...
		foreach ($columns as $key => $value) {
			if (is_int($key)) {
				...
				$column_key = !empty($key_match['alias']) ? $key_match['alias'] : $key_match['column'];
	...
	public function select($table, $join, $columns = null, $where = null): bool|array
    {
		$map = [];
		$stack = [];
		$column_map = [];
	...
	public function insert($table, $datas): bool|\PDOStatement
    {
		$stack = [];
		$columns = [];
		$fields = [];
	...
	public function update($table, $data, $where = null): bool|\PDOStatement
    {
		$fields = [];
		$map = [];
		foreach ($data as $key => $value) {
	...
	public function delete($table, $where): bool|\PDOStatement
    {
		$map = [];
		return $this->exec('DELETE FROM ' . $this->tableQuote($table) . $this->whereClause($where, $map), $map);
	}

	public function replace($table, $columns, $where = null): bool|\PDOStatement
    {
	...
	public function last(): array|string|null
    {
		$log = end($this->logs);
