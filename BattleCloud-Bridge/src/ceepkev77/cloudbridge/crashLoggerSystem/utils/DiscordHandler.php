...
namespace ceepkev77\cloudbridge\crashLoggerSystem\utils;
...
class DiscordHandler {
	...
	public function submit(): void{
		...
		if($result->getCode() !== 204){
			...
			Server::getInstance()->getLogger()->warning("Crash dump possibly not sent; Discord webhook api returned an unexpected http status code: ".$result->getCode()."\nPayload: ".$webhookData["payload_json"]."\n"."\nBody:\n".$result->getBody());
		}
	}
	...
	private function announceCrash(string $serverFolder): void{
		try{
			...
			Server::getInstance()->getPluginManager()->getPlugin("CloudBridge")->getLogger()->error("Error during crash announcement in file ".$e->getFile()." on line ".$e->getLine().": ".$e->getMessage());
		}
	}
	...
	private function getEmbedData(): array{
		...
		return [
			...
			"fields" => $fields,
			"footer" => [
				"text" => "Sent by CrashLogger v".Server::getInstance()->getPluginManager()->getPlugin("CloudBridge")->getDescription()->getVersion()
			]
		];
	}
}
