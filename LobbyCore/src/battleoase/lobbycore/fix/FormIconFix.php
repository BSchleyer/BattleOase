...
namespace battleoase\lobbycore\fix;
...
class FormIconFix implements Listener
{
	/** @var Closure[][] */
		...
		foreach($event->getPackets() as $packet){
			if($packet instanceof ModalFormRequestPacket){
				foreach($event->getTargets() as $target){
					BattleCore::getInstance()->getScheduler()->scheduleDelayedTask(new ClosureTask(function() use($target) : void{
						$player = $target->getPlayer();
						if($player !== null && $player->isOnline()){
							$this->onPacketSend($player, function() use($player, $target) : void{
								if($player->isOnline()){
									$times = 5; // send for up to 5 x 10 ticks (or 2500ms)
									BattleCore::getInstance()->getScheduler()->scheduleRepeatingTask(new ClosureTask(static function() use($player, $target, &$times) : void{
